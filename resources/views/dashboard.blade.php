                            ...
                            <div class="col-md-6">
                                <div>
                                    <ul class="mohasba-list">
                                        ...
                                        <li><i class="fa-solid fa-box-open"></i> <a
                                                href="{{ route('change.sites', $site->site_id) }}">{{ $site->site->name_ar }}</a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                ...
                <div class="row  pb-4 brdr">
                    <div class="col-md-6">
                        ...
                        <div>
                            <ul class="mohasba-list">
                                @if (count($Product) == 1)
                                    <li><i class="fa-solid fa-box-open"></i> <a href="{{ route('Product.tenant') }}"> لديك
                                            {{ count($Product) }} منتج . اضغط هنا للمزيد </a></li>
                                @elseif(count($Product) > 1)
                                    <li><i class="fa-solid fa-box-open"></i> <a href="{{ route('Product.tenant') }}"> لديك
                                            {{ count($Product) }} منتجات . اضغط هنا للمزيد </a></li>
                                @else
                                    <li><i class="fa-solid fa-box-open"></i> <a href="{{ route('Product.tenant') }}">ليس
                                            لديك أي منتجات، لإضافة منتج</a></li>
                                @endif

                                @if (count($Client) == 1)
                                    <li><i class="fa-solid fa-box-open"></i> <a href="{{ route('client.create') }}"> لديك
                                            {{ count($Client) }} عميل اضغط هنا للمزيد</a></li>
                                @elseif (count($Client) > 1)
                                    <li><i class="fa-solid fa-box-open"></i> <a href="{{ route('client.create') }}"> لديك
                                            {{ count($Client) }} عملاء اضغط هنا للمزيد</a></li>
                                @else
                                    <li><i class="fa-solid fa-users users-icon "></i> <a
                                            href="{{ route('client.create') }}">ليس لديك أي عملاء، اضغط هنا لإضافة عميل</a>
                                    </li>
                                @endif

                                @if (count($Supplier) == 1)
                                    <li><i class="fa-solid fa-box-open"></i> <a href="{{ route('Supplier.create') }}"> لديك
                                            {{ count($Supplier) }} مورد . اضغط هنا للمزيد</a></li>
                                @elseif (count($Supplier) > 1)
                                    <li><i class="fa-solid fa-box-open"></i> <a href="{{ route('Supplier.create') }}"> لديك
                                            {{ count($Supplier) }} موردين . اضغط هنا للمزيد</a></li>
                                @else
                                    <li><i class="fa-solid fa-handshake-simple"></i> <a
                                            href="{{ route('Supplier.create') }}">ليس لديك أي موردين، اضغط هنا لإضافة
                                            مورد</a></li>
                                @endif

                                @if (count($PurchaseInvoices) > 0)
                                    <li><i class="fa-solid fa-box-open"></i> <a
                                            href="{{ route('Purchase_Invoices.create') }}"> لديك
                                            {{ count($PurchaseInvoices) }} فاتورة مشتريات اضغط هنا للمزيد</a></li>
                                @else
                                    <li><i class="fa-solid fa-file-circle-check"></i> <a
                                            href="{{ route('Purchase_Invoices.create') }}">ليس لديك أي فواتير مشتريات، اضغط
                                            هنا لإضافة فاتورة مشتريات</a></li>
                                @endif

                                @if (count($sales_invoices) > 0)
                                    <li><i class="fa-solid fa-box-open"></i> <a
                                            href="{{ route('sales_invoices.create') }}"> لديك {{ count($sales_invoices) }}
                                            فاتورة مبيعات </a></li>
                                @else
                                    <li><i class="fa-solid fa-file-invoice-dollar"></i> <a
                                            href="{{ route('sales_invoices.create') }}">ليس لديك أي فواتير مبيعات، اضغط هنا
                                            لإضافة فاتورة مبيعات اضغط هنا للمزيد</a></li>
                                @endif

                                @if (count($Site) > 0)
                                    <li><i class="fa-solid fa-box-open"></i> <a href="{{ route('Site.index') }}"> لديك
                                            {{ count($Site) }} موقع اضغط هنا للمزيد</a></li>
                                @else
                                    <li><i class="fa-solid fa-location-arrow"></i> <a href="{{ route('Site.create') }}">
                                            مواقع. اضغط هنا لإضافة المزيد</a></li>
                                @endif
...
@section('script')
    ...
    <script>
        ...
        var data = {
            ...
            labels: ['{{ 100 - $resultPaymentsDesSales }} المبالغ المعلقة',
                '{{ $resultPaymentsDesSales }} المبالغ المتأخرة',
            ]
        };
        ...
        var data_2 = {
            ...
            labels: ['{{ 100 - $resultPaymentsDesMosh }} المبالغ المعلقة',
                '{{ $resultPaymentsDesMosh }} المبالغ المتأخرة',
            ]
        };
