@extends('layouts.vertical', ['title' => 'إنشاء إشعار مدينة'])
@section('content')
@section('css')
    ...
    <div class="row">
        ...
        <div class="col-lg-12 mt-3">
            <ul class="d-flex align-content-center">
                ...
                <li class="text-dark">
                    <i class="fa fa-angle-double-left mx-2 "></i><a href=""> إشعار مدينة</a>
                </li>
                <li class="text-primary">
                    <i class="fa fa-angle-double-left mx-2 "></i><a href=""> إنشاء إشعار مدينة </a>
                </li>
            </ul>
        </div>
...
<section>
    <div class="d-flex justify-content-sm-end mx-5">
        <button class="btn btn-primary mx-2"> <a href="{{ route('ReturnsPurchase_Invoices.index') }}" class="text-light">رجوع</a></button>
    </div>
    <div class="container my-3 max-con">
        <div class="row">
            <div class="col-md-12 hi-mohasba">

                <h4 class="mx-4"> إنشاء إشعار مدينة</h4>
            </div>

        </div>
        

        <form id="myForm" class="row  pb-4 brdr" action="{{ route('ReturnsPurchase_Invoices.create.post') }}" method="post">
            @csrf
                              ...
                              <tbody>
                                <tr>
                                  ...
                                  <td class="px-2 td-ftora">        
                                    <input name="code"type="text" class="form-control  my-2 tab-input" value="DBN {{$count}}"  id="code">
                                ...
                                <tr>
                                  <td colspan="4" >                      
                                    <h6 class="my-2 pe-1 input-w-custom">فاتورة المشتريات الاصلية<span class="star">*</span>
                                    </h6>
                                  </td>
                                  <td class="px-2 td-ftora">
                                    <select  class="form-select  form-select-lg my-2" name="purchase_invoice_id" id="ReturnsPurchase_Invoices-select1">
                                        <optgroup>
                                ...
                                <!--            <option value="{{ $salesperson->id }}" {{ old('id_salesperson') == $salesperson->id ? 'selected' : '' }}>-->
                                <!--            {{ $salesperson->name_ar }}-->
                                <!--        </option>-->
                                <!--         @endforeach-->
                                <!--    </select>-->
                        ...
                        ">
                              ...
                              <tbody>
                                ...
                                <tr>
                                  ...
                                  <td class="px-2 td-ftora">
                                    <select class="form-select  form-select-lg my-2" name="Note">
                                      <optgroup>
                                ...
                                <tr>
                                  ...
                                  <td class="px-2 td-ftora">
                                    <select class="form-select  form-select-lg my-2" name="" id="" onchange="checkinStore()">
                                         <option value=""  >محمود</option>
                                         <option value=""  >محمد</option>
                                         <option value=""  >عادل</option>
                                    </select>
                                  </td>
                                </tr>
                              </tbody>
...
<script>
    $(document).ready(function() {
        ...
        $('#Client-select1').on('change', function() {
            ...
            var employeeSelect = $('#ReturnsPurchase_Invoices-select1');

            // Clear the previous employee options
            employeeSelect.empty();

            // Add the default option
            employeeSelect.append('<option value="">اختار الفاتورة الخاصه بهذا المورد</option>');

            // If a branch is selected
            if (ClientId) {
                // Make an AJAX request to get the employees for the selected branch
                $.ajax({
                    url: '/dashboard/getAllInvolice/with/Supplier', // Replace with the actual endpoint URL that retrieves employees based on a branch ID
                    type: 'GET',
...
var PurchaseInvoiceDetailsTable = null
    ...
    // When the select element changes
        $('#ReturnsPurchase_Invoices-select1').on('change', function()
        {
            // drow the table
            // var ClientId = ;
            // alert("Get Data Success.")
            // var client_id = $('#Client-select1').val();;
            ...
            $.ajax({
                url: '/dashboard/getAllInvolice/with/id/Supplier', // Replace with the actual endpoint URL that retrieves employees based on a branch ID
                type: 'GET',
                data: { id: $(this).val()},
                success: function(response) {
                    var site          =  document.getElementById("site-id");
                    site.value = response.site_id
                }
           ...
           var site          =  document.getElementById("site-id").value;
            $.ajax({
                url: '/dashboard/getDetilaPurchaseInvoice/with/Client', // Replace with the actual endpoint URL that retrieves employees based on a branch ID
                type: 'GET',
                data: { id: $(this).val() },
                success: function(response) {
                    ...
                    $('#ReturnsPurchase_Invoices-select1').prop('disabled', false);
                    console.log(response)
                    $.each(response, function(key, value) {
                        // Create the <tr> element and append it to the table
                        var row = $("<tr>").appendTo(table);
                        
                        // Append the <th> and <td> elements to the row
                        row.append(`
                            <th class="text-center p-2">${counter + 1}</th>
                            <td class="text-center p-1" colspan="2" style="width: 90px;">
                                
                                <select id="product_${id}"  onchange="myFunction(${id})" name="test[]"  class="mt-2 form-select" style="height: 40px;">
                                    <optgroup>
                            ...
                            <td class="text-center" colspan="2" style="width: 30px;">
                                <input id="qun_product_${id}" onfocusout="result(${id})" name="test[]" type="text" class="form-custom my-2" value="${value.qun}" >
                                <select  id="dec_product_${id}"   onchange="resultUnit(${id})" name="test[]"   class="form-custom my-2" >
                                    <option value="${value.product.sel}-${value.product.unit.id}-1">${value.product.unit.name}</option>
                            ...
                            <td class="text-center" colspan="1" style="width: 125px;" >
                                <input id="price_unit_product_${id}" onfocusout="result(${id})"   name="test[]" type="text" class="form-custom-2 my-2" value="${value.price_unit}" style="width:130px;">
                            </td>
                            <td class="text-center" colspan="1" style="width: 30px;">
                                <input id="inc_desc_product_${id}" onclick="result(${id})"  class="form-check-input mt-3" type="checkbox">
                                
                                <input id="inc_input_product_${id}"  name="test[]" class="form-check-input mt-3" type="hidden" value="${value.withDescunt}">
                                
                            </td>
                            <td class="text-center" colspan="2" style="width: 30px;">
                                <input type="text" class="form-custom my-2" value="${value.descunt ?? 0}" id="desc_product_${id}" onfocusout="result(${id})"   name="test[]">
                                <select class="form-select form-select-lg" style="display: initial; width: 40%; height: 40px;" id="desc_product_op_${id}" onchange="result(${id})" name="test[]">
                                    <option value="0">قيمة</option>
                                    <option value="1">%</option>
                                </select>
                            </td>
                            <td class="text-center" colspan="1" style="width: 125px;" >
                                <input id="price_before_tax_product_${id}"   name="test[]" type="text" class="form-custom-2 no-cursor my-2" value="${value.price_before.toFixed(2)}" style="width:130px;">
                            </td>
                            <td class="text-center" colspan="1" style="width: 90px;">
                                <select class="form-select my-2 form-select-lg" style="display: initial; width: 90%; height: 40px;"  id="product_tax_${id}" onchange="result(${id})"   name="test[]">
                                    <option ${value.tax == 0 ? 'selected' : ''} value="0">0</option>
                                    <option ${value.tax == 15 ? 'selected' : ''} value="15">15</option>
                                </select>
                            </td>
                            <td class="text-center" colspan="1" style="width: 125px;">
                                <input id="Tax_value_${id}" name="test[]" type="text" class="form-custom-2 no-cursor my-2" value="${value.tax_value}" style="width:130px;">
                            </td>
                            <td class="text-center" colspan="1" style="width: 125px;">
                                <input type="text" class="form-custom-2 no-cursor my-2" value="${value.price_after.toFixed(2)}" name="test[]" id="Total_${id}" style="width:130px;">
                            </td>
                            <td class="text-center" colspan="1" style="width: 75px;">
                    ...
                    var inc_desc_product       =  document.getElementById("inc_desc_product_"+id);
                    var desc_product_op        =  document.getElementById("desc_product_op_"+id);
                    if(value.withDescunt == 'on') {
                        inc_desc_product.checked = true
                    } else {
                        inc_desc_product.checked = false
                    }
                    var pUnits        =  document.getElementById("dec_product_"+id);
                    // console.log(value.product.product_units);
                    const productUnit = $("#dec_product_" + id);
                    
                    $.each(value.product.product_units, function(key1, value1) {
                        productUnit.append(`<option value="${value1.price_sell}-${value1.id_unit}-${value1.counter_of_unit}">${value1.unitproduct_unit.name}</option>`);    
                    });
                    console.log(value.price_unit_id)
                    console.log("---------------------------------")
                    console.log(pUnits.value)
                  pUnits.value              = value.price_unit_id;
                  product.value             = value.product_id;
                  desc_product_op.value     = value.type_descount;
                  productUnit.value         = value.price_unit_id;
                  Total_value[id]           = parseFloat(value.price_after);
                  Total_before[id]          = parseFloat(value.price_before);
                  Total_Tax[id]             = parseFloat(value.tax_value);
                  id += 1
                  getSum();
                    });
                }
