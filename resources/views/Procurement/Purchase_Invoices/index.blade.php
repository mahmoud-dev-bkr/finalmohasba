@extends('layouts.vertical', ['title' => 'فاتورة مشتريات'])
@section('content')
      <div class="container-fluid">
                  ...
                  <div class="col-lg-12 mt-3">
                     <ul class="d-flex align-content-center">
                        ...
                        <li class="text-primary">
                           <i class="fa fa-angle-double-left mx-2 "></i><a href="Purchase_orderss.html">فاتورة مشتريات</a>
                        </li>
                     </ul>
                  </div>
               </div>
            ...
            <section>
               <div class="d-flex justify-content-sm-end mx-5">
                   @can('create_PurchaseInvoices')
                    <button class="btn btn-primary mx-2">  
                        <a href="{{ route('Purchase_Invoices.create') }}"  class="text-light">اضافة فاتورة مشتريات</a> 
                    ...
                    <button class="btn btn-primary mx-2">
                        <a href="{{ route('ReturnsPurchase_Invoices.create') }}" class="text-light">إشعارات مدين</a>
                        <i class="fa-solid fa-plus"></i>
                    </button>
                    <button class="btn btn-primary mx-2">
                        <a href="{{ route('Supplierbond.create') }}" class="text-light"> انشاء سند مورد </a>
                        <i class="fa-solid fa-plus"></i>
                    </button>
                    <!--{{-- <button class="btn btn-primary mx-2">استيراد قائمة فاتورة مشتريات  -->
               ...
               <div class="container my-3 max-con">
                  <div class="row">
                     <div class="col-md-12 hi-mohasba">
                        <h4 class="mx-4">فاتورة مشتريات</h4>
                     </div>
                  </div>
                                    ...
                                    <div class="d-flex justify-content-sm-center PurchaseInvoices-form">
                                        ...
                                        <select class="form-control w-100 select-fillter status" name="" id="">
                                            <optgroup>
                                                ...
                                                <option value="2">ملغيه</option>
                                                <option value="1"> يتم المراجهة </option>
                                                <option value="3"> تمت الموافقه عليها  </option>
                                                <option value="4"> دفعت بالكامل    </option>
                                                <option value="5"> دفعت جزء منها    </option>
                    ...
                    <div class="row bg-light pb-4 brdr">
                        <div class="col-md-12 PurchaseInvoicess ">
                            <div>
                                ...
                                <h1 class="my-3">ليس لديك أي فاتورة مشتريات</h1>
                                  
                                <p class="text-secondary my-5"> يتيح لك محاسبة خاصية انشاء فواتير المشتريات وهي وثائق تجارية صادره من البائع للمشتري تبين المنتجات او الخدمات المقدمة وكمياتها واسعارها</p>
                                <button class="btn btn-primary mx-2 "> <a href="{{ route('Purchase_Invoices.create') }}" class="text-light">اضافة فاتورة مشتريات</a>  <i class="fa-solid fa-plus"></i>
                            </div>
                        </div>
                    </div>
              @endif
...
@section('script')
    ...
    function reloadData(code,name, status,date,start_date,end_date, site) {
            ...
            // alert(site)
            var url = "{{ route('getPurchase_InvoicessData') }}?code=" + code+"&name="+ name +"&status="+ status +"&date="+ date +"&start_date="+ start_date+"&end_date="+ end_date+"&site="+ site;
        ...
        function reset()
        {
            ...
            // $('.barcode-product').val() = ""
            // $('.item-product').val() = ""
            var url = "{{ route('getPurchase_InvoicessData') }}"
            PurchaseInvoicesTable.ajax.url(url).load();
        }
        ...
        function downloadExcel(code,name, status,date,start_date,end_date, site) {
            ...
            const url = '{{ route("ExportPurchase_Invoices") }}?code=' + code+"&name="+ name +"&status="+ status +"&date="+ date +"&start_date="+ start_date+"&end_date="+ end_date+"&site="+ site;
