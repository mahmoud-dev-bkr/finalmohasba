@extends('layouts.vertical', ['title' => 'سندات الموردين'])
@section('content')
      <div class="container-fluid">
                  ...
                  <div class="col-lg-12 mt-3">
                     <ul class="d-flex align-content-center">
                        ...
                        <li class="text-primary">
                           <i class="fa fa-angle-double-left mx-2 "></i><a href="clients.html">سندات الموردين</a>
                        </li>
                     </ul>
                  </div>
               </div>
            ...
            <section>
               <div class="d-flex justify-content-sm-end mx-5">
                   @can('create_Supplierbond')
                    <button class="btn btn-primary mx-2">  
                        <a href="{{ route('Supplierbond.create') }}" class="text-light">اضافة سندات الموردين</a> 
                        <i class="fa-solid fa-plus"></i>
                    </button> 
                    @endcan
                    <button class="btn btn-primary mx-2">
                        <a href="{{ route('ExportSupplierbonds') }}" class="text-light">تحميل ملف اكسل</a>
                        <i class="fa-solid fa-plus"></i>
                    </button>  
                    <!--<button class="btn btn-primary mx-2">استيراد قائمة سندات الموردين  -->
               ...
               <div class="container my-3">
                  <div class="row">
                     <div class="col-md-12 hi-mohasba">
                        <h4 class="mx-4">سندات الموردين</h4>
                     </div>
                  </div>
                  @if (count($Supplierbond) > 0)
                    ...
                    <section>
                        ...
                        <div class="col-md-3"></div>
                                ...
                                <div class="row">
                                    ...
                                    <button onclick="reloadData($('.code').val(), $('.name').val(), $('.status').val(), $('.date').val(), $('.start-date').val(), $('.end-date').val())" class="btn btn-primary mx-1"><i class="fa-solid fa-magnifying-glass"></i> بحث</button>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="responsive-scroll">
                                    <table id="SupplierbondsTable" class="table text-center">
                                        <thead class="table-head">
                                        <tr>

                                            <th scope="col">كود</th>
                                            <th scope="col">اسم مورد</th>
                    ...
                    <div class="row bg-light pb-4 brdr">
                        <div class="col-md-12 clients ">
                            <div>
                                ...
                                <h1 class="my-3">ليس لديك أي سندات الموردين</h1>
                                <p class="text-secondary my-5">يوفر محاسبة صفحة خاصة بسندات الموردين للمساهمة في تسهيل التعاملات مع سندات الموردين وملخص لبياناتهم.</p>
                                <button class="btn btn-primary mx-2 "> <a href="{{ route('Supplierbond.create') }}" class="text-light">اضافة سندات الموردين</a>  <i class="fa-solid fa-plus"></i></button> <button class="btn btn-primary">استيراد قائمة سندات الموردين  <i class="fa-solid fa-right-to-bracket mx-1"></i></button>
                            </div>
                        </div>
                    </div>
                  @endif
...
@section('script')
    ...
    function reloadData(code,name, status,date,start_date,end_date) {
            ...
            var url = "{{ route('getSupplierbondsData') }}?code=" + code+"&name="+ name +"&status="+ status +"&date="+ date +"&start_date="+ start_date+"&end_date="+ end_date;
            SupplierbondsTable.ajax.url(url).load();
        }
        function reset()
        {
            ...
            // $('.barcode-product').val() = ""
            // $('.item-product').val() = ""
            var url = "{{ route('getSupplierbondsData') }}"
            SupplierbondsTable.ajax.url(url).load();
        }
        let SupplierbondsTable = null

        function setSupplierbondsDatatable() {
            var url = "{{ route('getSupplierbondsData') }}";
            SupplierbondsTable = $("#SupplierbondsTable").DataTable({
                processing: true,
                serverSide: true,
                dom: 'Blfrtip',
