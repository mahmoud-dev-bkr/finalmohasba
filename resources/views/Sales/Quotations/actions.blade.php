@switch($type)
    @case('action')
        <div>
            <ul>
                @if (canPerission('show_Quotation'))
                    <a href="{{ route('Quotation.show', $Purchase_orders->id) }}">
                        <li><i class="fa-solid fa-eye"></i></li>
                    </a>
                @endif
                @if (canPerission('done_Quotation'))
                    @if ($Purchase_orders->status == 4)
                        <a onclick="showErrorNotDone()">
                            <li><i class="fas fa-sync"></i></li>
                        </a>
                    @elseif($Purchase_orders->status == 3)
                        <a onclick="showErrorDelete()">
                            <li><i class="fas fa-sync"></i></li>
                        </a>
                    @else
                        <a href="{{ route('Quotation.done', $Purchase_orders->id) }}">
                            <li><i class="fas fa-sync"></i></li>
                        </a>
                    @endif
                @endif
                ...
                @if (canPerission('status_Quotation'))
                    @if ($Purchase_orders->status == 4)
                        <form action="{{ route('Quotation.status', $Purchase_orders->id) }}" method="post">
                            ...
                            <button class="action-icon notdone btn  btn-sm text-white" type="submit">
                                <li><i class="material-icons">x</i></li>
                            </button>
                        </form>
                    @elseif($Purchase_orders->status == 3)
                ...
                @if (canPerission('delete_Quotation'))
