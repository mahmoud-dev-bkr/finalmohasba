        ...
        <div class="row">
            ...
            <div class="col-lg-12 mt-3">
                <ul class="d-flex align-content-center">
                    ...
                    <li class="text-dark">
                        <i class="fa fa-angle-double-left mx-2 "></i><a href="{{ route('Quotation.index') }}"> فاتورة مبيعات</a>
                    </li>
        ...
        <div class="container my-3 max-con">
            <div class="row">
                <div class="col-md-12 hi-mohasba">
                    ...
                    <h4 class="mx-4"> اضافة فاتورة مبيعات</h4>
                </div>

            </div>
            <!--<div class="row bg-light pb-4 brdr">-->

                <form id="myForm" class="row  pb-4 brdr" action="{{ route('Quotation.done.post', $PurchaseInvoices->id) }}" method="post">
                    ...
                    <div class="col-md-6 ">
                        ...
                        <div class="form my-5">
                            ...
                            <div class="d-flex align-content-center justify-content-sm-between">
                                <label class="mt-3 ml-5"> المرجع</label><input  id="code"  value="INV {{ $count }}" name="code" type="text" class="form-control w-75 my-2">
                                <input name="done" type="text" class="form-control w-75 my-2" value="1" id="done" hidden>
                            </div>

                            <div class="d-flex align-content-center justify-content-sm-between">
                                <label class="mt-3 ml-5">الوصف</label>
                                <select class="form-control w-75" name="Note" >
                                    <optgroup>
                                        <option {{ $PurchaseInvoices->description == 'نقدي' ? 'selected' : '' }} value="نقدي">نقدي</option>
                                        <option {{ $PurchaseInvoices->description == 'اجله'  ? 'selected' : '' }} value="اجله">اجله</option>
                                    </optgroup>
                                </select>
                            </div>
                                ...
                                <div class="d-flex align-content-center justify-content-sm-between">
                                <label class="mt-3 ml-5"> تاريخ الإصدار</label><input value="{{ $PurchaseInvoices->Release_Date }}" name="Date_start" type="date" class="form-control w-75 my-2" id="Date_start_id" >
                                </div>
                            ...
                            <div class="d-flex align-content-center justify-content-sm-between">
                                <label class="mt-3 ml-5"> تاريخ الاستحقاق </label><input value="{{ $PurchaseInvoices->due_date }}" name="Date_end" type="date"
                                    class="form-control w-75 my-2" id="date_end_id">
                            </div>



                            <div class="d-flex align-content-center justify-content-sm-between">
                                <label class="mt-3 ml-5"> تاريخ التوريد </label><input type="date" value="{{ $PurchaseInvoices->Supply_date }}" name="Date_Groce_Period"
                                    class="form-control w-75 my-2" id="Date_Groce_Period">
                            </div>
                            <div class="d-flex align-content-center justify-content-sm-between my-3">
                                <label class="mt-3 ml-5">*الموقع</label>
                                <select  class="form-control w-75" name="site_id" id="site-id"  onchange="checkinStore()">
                                    <optgroup>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function() {
       ...
       @foreach ($QuotationDetails as $QuotationDetails)
           ...
           <th class="text-center p-2">${counter}</th>
                ...
                <td class="text-center" colspan="2" style="width: 30px;">
                    ...
                    <select id="dec_product_${id}" name="test[]" onchange="resultUnit(${id})" class="form-custom my-2">
                        <option value="{{ $productQ->sel }}-{{ $unit->id }}">{{ $unit->name }}</option>
                        @foreach ($productUnit as $pu)
                            @php
                                $unitPro = App\Uint::where('id', $pu->id_unit)->first();
                            @endphp
                            <option {{ $QuotationDetails->price_unit_id == ( $pu->price_sell - $pu->id_unit ) ? 'selected' : '' }} value="{{ $pu->price_sell }}-{{ $pu->id_unit }}">{{ $unitPro->name }}</option>
                        @endforeach
                    </select>
                    <span  id="errorMessage_${id}" class="error-message">الكمية غير متوافرة</span>
                    <span  id="qunMessage_${id}"   class="qun-message"></span>n>
        ...
        // productUnit           = document.getElementById("dec_product_" + id);
        // @php
            // $result = $QuotationDetails->price_unit . "-" . 	$QuotationDetails->unit_id ;
        // @endphp
        // productUnit.value     = ;
...
function checked(index) {
  ...
  $.get(ProductURL, function(data) {
    var qunWithUnit = data.qun / price[2];
    QunChek(data, qunInput, qunWithUnit, errorMessage, qunMessage, index, iderror)

  });
}

function QunChek(data, qunInput, qunWithUnit, errorMessage, qunMessage, index, iderror){
    qunMessage.textContent = "Qun = " + qunWithUnit;
    // alert(iderror);
    if(index == 0) {
