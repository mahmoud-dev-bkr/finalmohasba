@extends('layouts.vertical', ['title' => 'إنشاء إشعار دائن'])
@section('content')
@section('css')
    ...
    <div class="row">
        ...
        <div class="col-lg-12 mt-3">
            <ul class="d-flex align-content-center">
                ...
                <li class="text-dark">
                    <i class="fa fa-angle-double-left mx-2 "></i><a href=""> إشعار دائن</a>
                </li>
                <li class="text-primary">
                    <i class="fa fa-angle-double-left mx-2 "></i><a href=""> إنشاء إشعار دائن </a>
                </li>
            </ul>
        </div>
    ...
    <div class="container my-3 max-con">
        <div class="row">
            <div class="col-md-12 hi-mohasba">
                ...
                <h4 class="mx-4"> إنشاء إشعار دائن</h4>
            </div>

        </div>
        

        <form id="myForm" class="row  pb-4 brdr" action="{{ route('ReturnsSalesInvoices.create.post') }}" method="post">
            @csrf
                              ...
                              <tbody>
                                <tr>
                                  ...
                                  <td class="px-2 td-ftora">        
                                    <input name="code"type="text" class="form-control  my-2 tab-input" value="CRN {{$count}}"  id="code">
                                ...
                                <tr>
                                  <td colspan="4" >                      
                                    <h6 class="my-2 pe-1 input-w-custom">فاتورة المبيعات الاصلية<span class="star">*</span>
                                    </h6>
                                  </td>
                                  <td class="px-2 td-ftora">
...
<script>
    $(document).ready(function() {
        ...
        $('#Client-select1').on('change', function() {
            ...
            // Add the default option
            employeeSelect.append('<option value="">اختار الفاتورة الخاصه بهذا العميل</option>');

            // If a branch is selected
            if (ClientId) {
                // Make an AJAX request to get the employees for the selected branch
                $.ajax({
                    url: '/dashboard/getAllInvolice/with/Client', // Replace with the actual endpoint URL that retrieves employees based on a branch ID
                    type: 'GET',
...
var PurchaseInvoiceDetailsTable = null
    ...
    // When the select element changes
        $('#ReturnsSalesInvoices-select1').on('change', function()
        {
            ...
            $.ajax({
                url: '/dashboard/getAllInvolice/with/id', // Replace with the actual endpoint URL that retrieves employees based on a branch ID
                type: 'GET',
                data: { id: $(this).val()},
                success: function(response) {
                    var site          =  document.getElementById("site-id");
           ...
           var site          =  document.getElementById("site-id").value;
            $.ajax({
                url: '/dashboard/getDetilaSales_invoices/with/Client', // Replace with the actual endpoint URL that retrieves employees based on a branch ID
                type: 'GET',
                data: { id: $(this).val() },
                success: function(response) {
